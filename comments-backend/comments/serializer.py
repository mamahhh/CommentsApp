...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["user", "author", "create_at", "update_at", "related_images"]
    ...
    def update(self, instance, validated_data):
        ...
        instance.save()
        qs = CommentImage.objects.filter(comment=instance)
        if keep_ids:
        ...
        if new_paths:
            existing_count = CommentImage.objects.filter(comment=instance).count()
            objs = [
                CommentImage(comment=instance, image_path=p, order=existing_count + i)
